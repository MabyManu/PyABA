...
def draw_heatmap(fixations, dispsize, imagefile=None, durationweight=True, alpha=0.5, savefilename=None, ax = None):
	...
	for i in range(0,len(fix['dur'])):
		...
		if (numpy.isnan(fix['x'][i])):
			x=numpy.NaN
		else:
			x = strt + int(fix['x'][i]) - int(gwh/2)
            
		if (numpy.isnan(fix['y'][i])):
 			y=numpy.NaN
		else:
	...
	# remove zeros
	lowbound = numpy.nanmean(heatmap[heatmap>0])
